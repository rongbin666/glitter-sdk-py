...
from glitter_sdk.core.signature_v2 import Descriptor
from glitter_sdk.core.signature_v2 import Single as SingleDescriptor
from glitter_sdk.core.tx import AuthInfo, SignerInfo, SignMode, Tx
...
class Key:
    ...
    def __init__(self, public_key: Optional[PublicKey] = None):
        ...
        if public_key:
            self.raw_address = bytes.fromhex(public_key.raw_address())
    ...
    def acc_address(self) -> AccAddress:
        """Glitter Bech32 account address. Default derivation via :data:`public_key` is provided.
        ...
        return AccAddress(get_bech("glitter", self.raw_address.hex()))
        # array = binascii.unhexlify(self.raw_pubkey)
        # words = [x for x in array]
        # words = self.raw_address
        # bech32_words = convertbits(words, 8, 5)
        # bech32_address = bech32_encode("glitter", bech32_words, Encoding.BECH32)
        # return bech32_address

    @property
    def val_address(self) -> ValAddress:
        """Glitter Bech32 validator operator address. Default derivation via :data:`public_key` is provided.
        ...
        return ValAddress(get_bech("glittervaloper", self.raw_address.hex()))

    @property
    def acc_pubkey(self) -> AccPubKey:
        """Glitter Bech32 account pubkey. Default derivation via :data:`public_key` is provided.
        ...
        return AccPubKey(get_bech("glitterpub", self.raw_pubkey.hex()))

    @property
    def val_pubkey(self) -> ValPubKey:
        """Glitter Bech32 validator pubkey. Default derivation via ``public_key`` is provided.
        ...
        return ValPubKey(get_bech("glittervaloperpub", self.raw_pubkey.hex()))
