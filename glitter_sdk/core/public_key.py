...
from glitter_proto.cosmos.crypto.ed25519 import PubKey as ValConsPubKey_pb
from glitter_proto.cosmos.crypto.multisig import LegacyAminoPubKey as LegacyAminoPubKey_pb
from glitter_proto.cosmos.crypto.secp256k1 import PubKey as SimplePubKey_pb
from glitter_proto.ethermint.crypto.v1.ethsecp256k1 import PubKey as EthPubKey_pb
...
from glitter_sdk.util.encrypt.eth.ethereum import PublicKey as EthPublicKey
...
class PublicKey(JSONSerializable, ABC):
    ...
    def from_proto(cls, proto: Any_pb):
        ...
        elif type_url==EthSimplePublicKey.type_url:
            return EthSimplePublicKey.from_proto(EthPubKey_pb)
    ...
    def from_data(cls, data: dict):
        ...
        if type_url == SimplePublicKey.type_url:
            ...
            return EthSimplePublicKey.from_data(data)
...
@attr.s
class EthSimplePublicKey(PublicKey):
    """Data object holding the SIMPLE public key component of an account or signature."""

    type_amino = "ethermint/PubKeyEthSecp256k1"
    """"""
    type_url = "/ethermint.crypto.v1.ethsecp256k1.PubKey"
    """Normal signature public key type."""
    prototype = EthPubKey_pb
    """"""
    ...
    @classmethod
    def from_data(cls, data: dict) -> EthSimplePublicKey:
        return cls(key=base64.b64decode(data["key"]))

    @classmethod
    def from_proto(cls, proto: EthPubKey_pb) -> EthSimplePublicKey:
        return cls(key=proto.key)

    @classmethod
    def from_amino(cls, amino: dict) -> EthSimplePublicKey:
        return cls(key=base64.b64decode(amino["value"]))

    def to_proto(self) -> EthPubKey_pb:
        return EthPubKey_pb(key=self.key)
    ...
    def raw_address(self) -> bytes:
        eth_public_key = EthPublicKey.from_bytes(self.key)
        return eth_public_key.raw_address()

    def address(self) -> str:
        return get_bech("glitter", self.raw_address().hex())
